...
module BenchResults = struct
  ...
  type t =
    ...
    ; slow_table: Table.t
    ; fast_table: Table.t
    ; slow_table_pdiff: Table.t
    ; fast_table_pdiff: Table.t }
end
...
let fetch_bench_results ~job_info () =
  ...
  in
  let get_table_data name = artifact_url name |> fetch_artifact in
  let parse_quantity table =
    let regexp = {|.*TOP \([0-9]*\)|} in
    if Helpers.string_match ~regexp table then
      Some (Int.of_string @@ Str.matched_group 1 table)
    else None
  in
  let open Lwt_result.Syntax in
  let get_table name =
    let* table = get_table_data name in
    let count = parse_quantity table in
    Lwt_result.return BenchResults.Table.{table; count}
  in
  let* summary_table = get_table "bench_summary" in
  let* slow_table = get_table "slow_table" in
  let* fast_table = get_table "fast_table" in
  let* slow_table_pdiff = get_table "slow_table_pdiff" in
  let* fast_table_pdiff = get_table "fast_table_pdiff" in
  Lwt_result.return
    BenchResults.
      {summary_table; slow_table; fast_table; slow_table_pdiff; fast_table_pdiff}

let bench_text results =
  (* Formatting helpers *)
  ...
  in
  let pp_table title table ~emoji =
    [ header2
      @@ f "%s %a %s:" emoji pp_count table.BenchResults.Table.count title
    ; code_wrap table.table ]
  in
  (* Document *)
  let BenchResults.
        { summary_table
    ...
    results
  in
  [ pp_table "Bench Summary" summary_table ~emoji:":checkered_flag:"
  ; pp_table "Slow Downs" slow_table ~emoji:":turtle:"
  ; pp_table "Speed Ups" fast_table ~emoji:":rabbit2:"
  ; pp_table "Slow Downs (%)" slow_table_pdiff ~emoji:":snail:"
  ; pp_table "Speed Ups (%)" fast_table_pdiff ~emoji:":eagle:" ]
  |> Caml.List.flatten |> String.concat ~sep:"\n"

let bench_comment ~bot_info ~owner ~repo ~number ~gitlab_url ~check_url
    (results : BenchResults.t) =
  let open Lwt_result.Syntax in
  let* id = GitHub_queries.get_pull_request_id ~bot_info ~owner ~repo ~number in
  (* Formatting heleprs *)
  ...
  let link_bullet text url ~emoji = f "- %s %s" emoji (link text url) in
  let pp_table title table ~emoji =
    String.concat ~sep:"\n"
      BenchResults.Table.[f "%s %s:" emoji title; code_wrap table.table]
  in
  let pp_table_details title table ~emoji =
    let open BenchResults.Table in
    details
      (f "%s %a %s" emoji pp_count table.count title)
      (code_wrap table.table)
  in
  (* For making sure we are using all the data *)
  let BenchResults.
        { summary_table
        ; slow_table
        ; fast_table
  ...
  (* Body of message *)
  let message =
    [ pp_table "Bench results" summary_table ~emoji:":checkered_flag:"
    ; pp_table_details "slow downs" slow_table ~emoji:":turtle:"
    ; pp_table_details "speed ups" fast_table ~emoji:":rabbit2:"
    ; pp_table_details "slow downs (%)" slow_table_pdiff ~emoji:":snail:"
    ; pp_table_details "speed ups (%)" fast_table_pdiff ~emoji:":eagle:"
    ; link_bullet "GitLab Bench Job" gitlab_url ~emoji:":chair:" ]
  in
  let summary_line =
    match check_url with
    | Ok url ->
        [link_bullet "Bench Check Summary" url ~emoji:":spiral_notepad:"]
    | _ ->
        []
  in
  let message = String.concat ~sep:"\n" (message @ summary_line) in
  GitHub_mutations.post_comment ~bot_info ~id ~message
...
let update_bench_status ~bot_info job_info (gh_owner, gh_repo) ~external_id
  ...
  match number with
  | None ->
      Lwt.return_error "No PR number provided for bench summary so aborting."
  | Some number -> (
      let* repo_id =
        GitHub_queries.get_repository_id ~bot_info ~owner:gh_owner ~repo:gh_repo
      in
      ...
      let create_check_run ~status ?conclusion ~title ?(text = "") () =
        GitHub_mutations.create_check_run ~bot_info ~name:context ~status
          ~repo_id ~head_sha:job_info.common_info.head_commit ?conclusion ~title
          ~details_url:gitlab_url ~summary ~text ~external_id ()
      in
      match state with
      | "success" ->
      ...
      | "created" ->
          Lwt_result.return "Bench job has been created, ignoring info update."
      | _ ->
          Lwt.return_error "Unknown state for bench job: %s" )
  ...
  | "coq/coq", "bench" ->
      ...
      let* msg =
        update_bench_status ~bot_info job_info (gh_owner, gh_repo) ~external_id
          ~number:pr_num
      in
      let msg = match msg with Ok msg -> msg | Error msg -> msg in
      Lwt_io.printlf "update_bench_status: %s" msg
  | _, _ -> (
